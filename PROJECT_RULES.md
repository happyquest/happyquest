...
### ワークフロー必須要件
- **ランナー**: `ubuntu-latest` 使用必須（安定性確保）
- **アクション版数**: 最新安定版指定（例：`actions/setup-python@v5`）
- **エラーハンドリング**: `continue-on-error: true` 設定
- **ファイル存在確認**: 実行前の必須ファイル確認実装

### 必須ワークフロー構成
1. **ansible-lint.yml**: Ansible構成管理品質確認
2. **ci-integration.yml**: 統合CI/CDパイプライン  
3. **quick-test.yml**: 高速ヘルスチェック

### ワークフロー品質基準
- **詳細ログ出力**: `=== Section Headers ===` 形式
- **条件分岐処理**: ファイル存在・環境確認
- **グレースフル・デグラデーション**: 部分失敗許容
- **デバッグ支援**: 初心者向け理解しやすいメッセージ

### トラブル事例管理

#### 必須ドキュメント作成
すべてのトラブル発生時に以下を作成：
1. **トラブル事例報告書** (`docs/TROUBLESHOOTING_[システム名].md`)
2. **作業報告書** (`docs/WORK_REPORT_YYYYMMDD.md`)
